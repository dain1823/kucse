...
void load_names(FILE* fp, int year_index, tNames* names) {
	...
	do {
		...
		sscanf(str, "%s %c %d", temp_name, &temp_sex, &temp_int);

		int check_exist = 0;
		if (names->len != 0) {
			for (int i = 0; i < names->len; ++i) {
				if ((strcmp(names->data[i].name, temp_name) == 0) && (names->data[i].sex == temp_sex)) {
					names->data[i].freq[year_index] = temp_int;
					check_exist = 1;
					break;
				}
			}
		...
		if (!check_exist) {
			...
			names->data[names->len].freq[year_index] = temp_int;
			++names->len;
		}
	} while (feof(fp) == 0);
}
